...
\end{figure}
式(\ref{eq:density_compare})を満たすような範囲$J$において数値計算を行う場合，密度の高い数値型$T_{2}$の方が計算の際に生じる誤差が少ない可能性がある．
次章で行う数値実験は，倍精度浮動小数点(Float64)よりも同じく64bitの固定小数点を用いて倍精度浮動小数点(Float64)よりも密度が高いと思われる範囲での数値計算を行った．

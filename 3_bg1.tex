...
そのうち，$m \ (m < N)$bitを小数部に割り当てた固定小数点数$a = a_{N-m-1} a_{N-m-2} \cdots a_1 a_0. a_{-1} \cdots a_{-m}$の値は，符号を考えない場合以下のようになる：
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw (2,1.35) node[fill=white] {$(N-m-1)$bits};
...
$s$を符号とすると，符号付き固定小数点数$a_{signed} = s a_{N-m-2} a_{N-m-3} \cdots a_1 a_0. a_{-1} \cdots a_{-m}$の値は以下のようになる：
\begin{align}
    a_{signed} &= (-1)^s \left( a_{N-m-2} \times 2^{N-m-2} + a_{N-m-3} \times 2^{N-m-3} + \cdots + a_{1} \times + 2^{1} a_{0} \times 2^{0} \right. \\
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \draw[<->,thick] (5,1.3) -- (16,1.3);
        \draw (3,1.35) node[fill=white] {$(N-m-2)$bits};
        \draw (11,1.35) node[fill=white] {$m$bits};
...
\end{figure}
単精度浮動小数点(Float32)は，指数部の8bitででは整数値で$0 \sim  255$の値を取るが$-127$のバイアスがある．
また，仮数部の値は暗黙に一つ上の位が$1$であるとして，小数点以下の位を表す表現方法を用いている．
よって，単精度浮動小数点で表せられる値$b_{32}$は，指数部の数を$e_{32}$，仮数部の各bitを$b_{-1}$，$b_{-2}$,...,$b{-23}$とすると，
\begin{align}
    b_{32} &= (-1)^{sign}(1 + b_{-1}\times 2^{-1} + b_{-2}\times 2^{-2} + \cdots + b_{-23}\times 2^{-23})\times 2^{e_{32} - 127} \\
        &= (-1)^{sign}\left(1 + \sum_{i = 1}^{23} b_{-i} 2^{-i}\right)\times 2^{e_{32} - 127}
\end{align}
となる．
...
\end{figure}
倍精度浮動小数点（Float64）は指数部の11bitで整数値$0 \sim  2047$を取るが$-1023$のバイアスがある．
...
\begin{align}
    b_{64} &= (-1)^{sign}(1 + b_{-1}\times 2^{-1} + b_{-2}\times 2^{-2} + \cdots + b_{-52}\times 2^{-52})\times 2^{e_{64} - 1023} \\
        &= (-1)^{sign}\left(1 + \sum_{i = 1}^{52} b_{-i} 2^{-i}\right)\times 2^{e_{64} - 1023}
\end{align}
となる．
...
\begin{table}[H]
    ...
    Float32 &　$\sum_{i=105}^{128}2^i$ & $-\sum_{i=105}^{128}2^i$& \\
    Float64 & $\sum_{i=972}^{1024}2^i$ & $-\sum_{i=972}^{1024}2^i$&
    \end{tabular}
...
それに対して(\ref{fig:floatpoint_image})のように浮動小数点数で表せる数で隣り合う数は隣の数までの感覚が一定ではない．
また，絶対値が$0$に近い場合
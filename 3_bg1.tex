...
\section{浮動小数点と固定小数点の表現方法および演算方法}
...
そのため，図\ref{fig:fixed_float_image}で固定小数点のグラフが浮動小数点のグラフの下に来ている範囲では固定小数点の方が最小単位が小さい数で演算を行え，誤差が少なくなるできると考える．
...
\section{固定小数点演算の実用性の検証のための誤差比較}
これまで，固定小数点演算と浮動小数点演算の利点や欠点について述べた．
現在の計算機では，浮動小数点が主流である．
浮動小数点の方が範囲が広く，指数部を調整することのよって扱える数の最小単位を変えることができるため計算による誤差も固定小数点に比べ少ないと考えられている．
一方で，固定小数点は扱える範囲内では小さい数が無視されてしまう情報落ちや計算の精度が落ちてしまう情報落ちが生じないという利点も存在する．
そのため，扱う数の範囲が小さい状況下では，固定小数点の方が浮動小数点よりも計算誤差を少なくできると考えた．
固定小数点での計算の誤差が浮動小数点の誤差と同程度に小さければ，固定小数点も実用性がある．
第\ref{chap:数値実験}章では，固定小数点と浮動小数点の誤差を比較し，固定小数点の実用性を検証した．
